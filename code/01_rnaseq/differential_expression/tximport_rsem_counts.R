...
# Read metadata first so its sample order defines the final matrix columns.
metadata <- read_tsv(metadata_path, show_col_types = FALSE)
if (!"sample_id" %in% names(metadata)) {
  stop("metadata is missing a sample_id column", call. = FALSE)
}
metadata <- metadata |> mutate(sample_id = trimws(sample_id))
...
if (anyDuplicated(sample_order)) {
  dupes <- unique(sample_order[duplicated(sample_order)])
  stop("duplicate sample_id values found in metadata: ",
       paste(dupes, collapse = ", "), call. = FALSE)
}

# Discover all RSEM gene-level outputs in the nf-core result directory.
rsem_files <- sort(list.files(
  rsem_dir,
...
if (anyDuplicated(sample_names)) {
  ...
  stop("duplicate RSEM sample names found: ",
       paste(dupes, collapse = ", "), call. = FALSE)
}

# Reorder files to match metadata exactly
file_lookup <- setNames(rsem_files, sample_names)
missing_samples <- setdiff(sample_order, names(file_lookup))
if (length(missing_samples) > 0) {
  stop("no RSEM files found for samples: ",
       paste(missing_samples, collapse = ", "), call. = FALSE)
}

files <- file_lookup[sample_order]

# Import gene-level expected counts directly from RSEM
txi <- tximport(files, type = "rsem", txIn = FALSE,
